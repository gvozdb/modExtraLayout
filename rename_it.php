<?php
/**
 * Rename script for modExtraLayout
 *
 ...
 */
$new_name = !empty($_REQUEST['name']) ? $_REQUEST['name'] : (!empty($argv[1]) ? $argv[1] : '');
$new_short_name = !empty($_REQUEST['short']) ? $_REQUEST['short'] : (!empty($argv[2]) ? $argv[2] : '');
$new_name_lower = strtolower($new_name);
$new_short_name = empty($new_short_name) ? $new_name_lower : $new_short_name;
    ...
    foreach ($items as $item) {
        ...
        if (is_dir($new_path)) {
            ...
            if ($item == 'home.class.php') {
                $content = str_replace($replace[0] . 'ManagerController', 'modExtraLayoutManagerController', $content);
            }
